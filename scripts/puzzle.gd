...
@export var water_center_x: float = 500.0

func _process(_delta):
	for i in 3:
		reflections[i].global_position = Vector2(
			water_center_x + (water_center_x - blocks[i].global_position.x),
			water_y + (water_y - blocks[i].global_position.y)
		)
		
		if reflections[i].global_position.distance_to(targets[i].global_position) < 30:
			reflections[i].modulate = Color(0, 1, 0)
		else:
			reflections[i].modulate = Color(1, 1, 1)
